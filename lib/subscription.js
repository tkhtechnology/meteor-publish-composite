...
class Subscription {
  ...
  /**
   * Clear all internal state to release references and prevent memory leaks.
   * Called when the publication stops.
   */
  cleanup () {
    debugLog('Subscription.cleanup', 'clearing docHash and refCounter')
    // Clear docHash to release all document copies
    Object.keys(this.docHash).forEach((key) => {
      delete this.docHash[key]
    })
    // Clear refCounter internal state
